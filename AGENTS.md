...
- **Deployment:** Netlify (frontend), Supabase (backend); env vars for Supabase URL/anon key
  live in Netlify environment settings, never committed. Netlify needs an SPA redirect rule
  (`_redirects` or `netlify.toml`: `/*  /index.html  200`) so client-side routes don't 404 on
  refresh/deep link.
