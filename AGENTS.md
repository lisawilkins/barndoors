...
- **Calendar:** Hand shift scheduling (recurring shifts, one-off events, vacations) is
  built as an **in-app view only** — `/hands/schedule` (`HandSchedule.jsx`), mirroring
  `/wranglers/schedule`. A subscribable `.ics` feed URL keyed to a per-profile
  `calendar_feed_token`, and a one-time `.ics` download, are still **planned but not
  built** — `profiles.calendar_feed_token` exists as a column and is generated for every
  profile, but no Supabase Edge Function reads it yet, and nothing in the frontend
  surfaces a feed URL or a download button. No Google/Apple Calendar API integration, no
...
- **Hands scheduling mirrors the Wrangler pattern, with no activity/horse concept.**
  ...
  `insert` statements inside the migration itself); managers can add more beyond the
  ...
  one-off is a freestanding event with its own **title**, **date**, free-text **time**,
  **notes**, and **multiple** assigned hands (`hand_shift_events` + join table
  `hand_shift_event_members`) — e.g. "Gymkhana, Sept 29, 8am, groom for event, Anne/Lisa/
  Sharon." There is no Hand equivalent of `wrangler_calendar_notes` (day/month standing
  notes) — out of scope. **Vacations** (`hand_vacations`) are a separate small table, not
  two columns on `profiles`, because a hand can have multiple separate vacation ranges on
  file at once (`start_date`/`end_date`, checked `end_date >= start_date`). Vacation is a
  **visual-only overlay**: a hand's entry on any day inside one of their vacation ranges is
  dimmed and marked with 🌴 by the schedule UI, but the underlying recurring/one-off row is
  never removed, skipped, or hidden — a manager still has to use the skip/remove flow
  separately if a specific shift needs to actually go away. The old `public.shifts` table
  (dead scaffolding — zero frontend references, no `.ics` Edge Function ever built despite
  `profiles.calendar_feed_token` existing) was dropped in the same migration that added
  these tables; see "Calendar" above and Part 5 of `barndoors-schema.md`.
...
- **Page content width has two tiers**, both centered, independent of the full-bleed
  ...
  default 800px. Used today by the Wrangler Schedule's Monthly view, the Hand Schedule's
  Monthly view (`HandSchedule.jsx`, `/hands/schedule`), and the Feed Schedule table
  report; reuse it for any future landscape view rather than inventing a new width scheme.
